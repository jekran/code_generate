...
import com.cg.dialog.CommonDialog;
import com.cg.utils.CodeGenerateUtils;
import com.cg.utils.MyNotifier;
import com.cg.utils.entity.enums.CommonTypeEnum;
...
public class CommonAction extends BaseAnAction {
    private String text;

    @Override
    public void actionPerformed(@NotNull AnActionEvent e) {
        this.init(e);
        text = e.getPresentation().getText();
        CommonDialog formTestDialog = new CommonDialog(e.getProject(), this, text);
    ...
    public void doGenerate(String moduleName, String author, String className) {
        /**
         * 参数
         */
        ...
        try {
            ...
            switch (text) {
                case "装饰模式":
                    cfg.setClassLoaderForTemplateLoading(getClass().getClassLoader(),"/templates/designpatterns/decorator/");
                    // 生成接口类
                    writePath = path + "/src/main/java/" + moduleName + "/" + "I" + className + ".java";
                    generateFile("decorator.interface.java.ftl", cfg, writePath, model);
                    // 目标实现类
                    writePath = path + "/src/main/java/" + moduleName + "/" + "origin" + className + ".java";
                    generateFile("decorator.origin.java.ftl", cfg, writePath, model);
                    // 装饰类
                    writePath = path + "/src/main/java/" + moduleName + "/" + className + "Decorator.java";
                    generateFile("decorator.decorator.java.ftl", cfg, writePath, model);
                    break;
//                case SG_ENUM:
//                    // 生成业务类
